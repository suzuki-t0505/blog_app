defmodule BlogAppWeb.AccountSettingsComponent do
  ...
  # phx-target={@myself}を追加することでこのComponentのhandle_eventを呼び出せる
  # ※phx-target={@myself}がない場合は親LiveViewのhandle_eventを呼び出す
  def render(assigns) do
    ~H"""
    <div>
      <.header class="text-center">
        Account Settings
        <:subtitle>Manage your account profile and email address and password settings</:subtitle>
      </.header>
  ...
  def handle_event("update_email", params, socket) do
    ...
    case Accounts.apply_account_email(account, password, account_params) do
      {:ok, applied_account} ->
        ...
        {:noreply, assign(socket, email_form_current_password: nil)}
  ...
  # 受け取った引数（{atom, value}）を親LiveViewにメッセージ送信する
