defmodule BlogApp.ArticlesTest do
  ...
  describe "articles" do
    ...
    test "create_article/1 with valid data creates a article" do
      valid_attrs = %{body: "some body", status: 42, submit_date: ~D[2023-05-16], title: "some title"}

      assert {:ok, %Article{} = article} = Articles.create_article(valid_attrs)
      assert article.body == "some body"
      assert article.status == 42
      assert article.submit_date == ~D[2023-05-16]
    ...
    test "update_article/2 with valid data updates the article" do
      article = article_fixture()
      update_attrs = %{body: "some updated body", status: 43, submit_date: ~D[2023-05-17], title: "some updated title"}

      assert {:ok, %Article{} = article} = Articles.update_article(article, update_attrs)
      assert article.body == "some updated body"
      assert article.status == 43
      assert article.submit_date == ~D[2023-05-17]
